...
`session_history` — the evaluator does NOT read that DB directly. The read logic is
split in two:

1. **Component (logic)** — in **fred-core**, next to the history store
   (`PostgresHistoryStore`, `SessionHistoryRow`): a `HistoryCaptureReader` class.
   Put in fred-core (not fred-runtime) because fred-core is imported by *both*
   fred-agents and control-plane, so the host can be either without coupling a
   service to fred-runtime. **DONE** (fred repo, branch `1874-...`).
2. **Endpoint (HTTP route)** — in an app that mounts the component: **fred-agents**
   (has Keycloak) **vs** control-plane (runtime owner leans here). Placement TBD;
   the component is host-agnostic so it moves without rewrite. The evaluator calls
   it over HTTP in M2M (same channel as `evaluate_url`). **TODO** (fred repo, #1874).
