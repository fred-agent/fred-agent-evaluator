...
def evaluate_scenario(
    ...
    *,
    ...
    result = evaluate_case_sync(
        base_url=base_url, request=request, judge=judge, access_token=access_token
    )

    rag_ok = all(c.passed for c in result.structural_checks)
    metrics_by_name = {
        m.name: {"score": m.score, "verdict": m.verdict, "explanation": m.explanation}
...
def main() -> int:
    ...
    if args.use_temporal:
        from fred_deepeval_cli.dataset_workflow import (
            run_with_temporal,
            run_with_temporal_server,
        )

        questions = _build_questions(dataset, args)
        if args.temporal_server:
            console.print(
                f"[dim]Mode Temporal — serveur : {args.temporal_server}[/dim]\n"
            )
            results = asyncio.run(
                run_with_temporal_server(questions, args.temporal_server)
            )
        else:
            console.print(
                "[dim]Mode Temporal — in-memory (pas de serveur requis)[/dim]\n"
            )
